...
Point update_pos(const Point &pos, GameState &state) {
  ...
  if (state.prev_direction != state.direction
    ...
    cur_pos.rotation = state.texture_angle;
  }
...
//    model = glm::scale(model, glm::vec3(0.99f));
    ...
    shader.set_float("in_rotation", glm::radians(p.rotation));
...
int main() {
  ...
  Shader shader("shaders/snake_shader.vs", "shaders/snake_shader.fs");
  Shader grid_shader("shaders/line_shader.vs", "shaders/line_shader.fs");
...
VertexData init_vertices() {
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, vertex_data.indices.size() * sizeof(uint32_t), &vertex_data.indices[0],
               GL_STATIC_DRAW);
